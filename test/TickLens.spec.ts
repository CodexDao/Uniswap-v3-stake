...
import { MockTimeNonfungiblePositionManager, TestERC20, TickLensTest, PairCreate } from '../typechain'
...
describe('TickLens', () => {
  ...
  const nftFixture: Fixture<{
    ...
    const { factory, tokens, nft, pairCreate } = await completeFixture(wallets, provider)
  ...
  beforeEach('load fixture', async () => {
    ; ({ factory, tokens, nft, pairCreate } = await loadFixture(nftFixture))
  })
