...
describe('NonfungiblePositionManager', () => {
  ...
  beforeEach('load fixture', async () => {
    ; ({ nft, factory, tokens, weth9, router, pairCreate } = await loadFixture(nftFixture))
  })
  ...
  describe('#createAndInitializePoolIfNecessary', () => {
    ...
    it('gas', async () => {
      await snapshotGasCost(
        pairCreate.createAndInitializePoolIfNecessary(
          tokens[0].address,
          tokens[1].address,
          FeeAmount.MEDIUM,
          encodePriceSqrt(1, 1)
  ...
  describe('#mint', () => {
    ...
    it('fails if cannot transfer', async () => {
      ...
      ).to.be.reverted//revertedWith('STF')
    })
  ...
  describe('#decreaseLiquidity', () => {
    ...
    it('cannot be called by other addresses', async () => {
      await expect(
        nft.decreaseLiquidity({ tokenId, liquidity: 50, amount0Min: 0, amount1Min: 0, deadline: 100000000000000 })
      ).to.be.revertedWith('Not approved')
    })

    it('decreases position liquidity', async () => {
      await nft.connect(other).decreaseLiquidity({ tokenId, liquidity: 25, amount0Min: 0, amount1Min: 0, deadline: 100000000000000 })
      const { liquidity } = await nft.positions(tokenId)
    ...
    it('is payable', async () => {
      await nft
        .connect(other)
        .decreaseLiquidity({ tokenId, liquidity: 25, amount0Min: 0, amount1Min: 0, deadline: 100000000000000, }, { value: 1 })
    })
    ...
    it('can decrease for all the liquidity', async () => {
      await nft.connect(other).decreaseLiquidity({ tokenId, liquidity: 100, amount0Min: 0, amount1Min: 0, deadline: 100000000000000, })
      const { liquidity } = await nft.positions(tokenId)
    ...
    it('cannot decrease for more than all the liquidity', async () => {
      await expect(
        nft.connect(other).decreaseLiquidity({ tokenId, liquidity: 101, amount0Min: 0, amount1Min: 0, deadline: 100000000000000, })
      ).to.be.reverted
    })
    ...
    it('gas partial decrease', async () => {
      await snapshotGasCost(
        nft.connect(other).decreaseLiquidity({ tokenId, liquidity: 50, amount0Min: 0, amount1Min: 0, deadline: 100000000000000, })
      )
    })

    it('gas complete decrease', async () => {
      await snapshotGasCost(
        nft.connect(other).decreaseLiquidity({ tokenId, liquidity: 100, amount0Min: 0, amount1Min: 0, deadline: 100000000000000, })
      )
    })
  })
  ...
  describe('multicall exit', () => {
    ...
    async function exit({
      ...
      const decreaseLiquidityData = nft.interface.encodeFunctionData('decreaseLiquidity', [
        { tokenId, liquidity, amount0Min, amount1Min, deadline: 100000000000000, },
      ])
